"""
Pytorch implementation of Convoluted seq2seq for AIS trajectory prediction
"""
import numpy as np
...
if __name__ == "__main__":
    ...
    #===============================
    x=os.getcwd()
    if not os.path.isdir(os.path.join(x, cf.savedir)):
        os.makedirs(os.path.join(x, cf.savedir))
        print('======= Create directory to store trained models: '+os.path.join(x, cf.savedir))
    else:
        print('======= Directory to store trained models: '+os.path.join(x, cf.savedir))
    utils.new_log(os.path.join(x, cf.savedir),"log")
    ...
    ## Model
    #===============================
    model = models.conv_seq2seq(cf, partition_model=None)
    ...
    trainer = trainers.Trainer(
        model, aisdatasets["train"], aisdatasets["valid"], cf, savedir=cf.savedir, device=cf.device)

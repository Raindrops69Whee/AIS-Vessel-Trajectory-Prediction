...
class DenseLayer(nn.Module):
    ...
    def __init__(self, in_channels, bn_size, growthrate, val):
        super().__init__()
        self.bn1=nn.BatchNorm1d(in_channels-val*growthrate, device=device)
        self.conv1 = nn.Conv1d(
            in_channels-val*growthrate, bn_size*growthrate, kernel_size=1, bias=False, device=device
        )
        self.bn2 = nn.BatchNorm1d(bn_size*growthrate, device=device)
        self.conv2 = nn.Conv1d(
            bn_size*growthrate, in_channels-val*growthrate, kernel_size=3, padding=1, bias=False, device=device
        )

    def forward(self, prev_features):
        # out1 = torch.cat(prev_features, dim=1)
        out1 = sum(prev_features)
...
class Dense_Block(nn.ModuleDict):
    def __init__(self, in_channels, n_layers, growthrate, bn_size):
        """
        ...
        for i in range(n_layers):
            layer = DenseLayer(in_channels + i * growthrate, bn_size, growthrate, i)
...
class TrAISformer(nn.Module):
    ...
    def __init__(self, config, partition_model = None):
        ...
        #     nn.Conv1d(config.max_seqlen, config.max_seqlen, kernel_size=7, padding=3, stride=1, bias=False),
        #     nn.BatchNorm1d(config.max_seqlen),
        #     nn.MaxPool1d(1, 1),
        #     Dense_Block(config.max_seqlen, 6, growthrate=32, bn_size=4),
        # )
...
#             pdb.set_trace()
    ...
    def forward(self, x, masks = None, with_targets=False, return_loss_tuple=False):
        ...
        dense_block = nn.Sequential(
            nn.Conv1d(fea.shape[1], fea.shape[1], kernel_size=7, padding=3, stride=1, bias=False, device=device),
            nn.BatchNorm1d(fea.shape[1], device=device),
            nn.MaxPool1d(1, 1),
            Dense_Block(fea.shape[1], 6, growthrate=32, bn_size=4),
        )
        # skip_output = dense_block(fea)
        # fea = self.blocks(fea)
        # output = skip_output+fea
        # output = self.ln_f(output) # (bs, seqlen, n_embd)
        ...
        # fea = self.ln_f(fea) # (bs, seqlen, n_embd)
        # skip_output = dense_block(fea)
        # output = skip_output+fea_original
        # logits = self.head(output) # (bs, seqlen, full_size) or (bs, seqlen, n_embd)
